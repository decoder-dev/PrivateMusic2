...
- VK API adapter and deterministic demo service with an original local tone;
...
For development, launch with the argument `--demo` or environment variable
`PRIVATE_MUSIC_DEMO=1`. The connection screen can store an existing test access
token in Keychain. Availability of `audio.*` methods depends on VK permissions;
...
The included GitHub Actions workflow runs on a macOS runner, tests the iOS
Simulator target, builds the arm64 iPhone target and packages
`PrivateMusic-2.0-unsigned.ipa`. Push the contents of this directory to a GitHub
repository and run the `Build unsigned IPA` workflow.

Download the `PrivateMusic-2.0-unsigned` workflow artifact when the job
...
- Playlist editing, lyrics and offline downloads are planned next.
